...
F# では、 @<kw>{let} キーワードを使って値と識別子を関連付けられます。
これを、値束縛と呼びます。
...
実は変数の定義は書く場所によって 2 つに分類できます。
 ...
 1. モジュールの中などに書いた場合の @<kw>{let} による@<strong>{値定義}
...
このように、@<kw>{in} に続けて式を書くのが本来の let 式です。
...
F# は @<kw>{軽量構文} という構文をサポートしており、let 式の @<kw>{in} を改行とインデントで代用できます。
...
この式は問題なくコンパイルできます。
しかし、次の式は then に続く式の型が int であるのに対して、else に続く式の型は float なので、両者の型が合わずにコンパイルエラーになります。
...
条件式には、 bool 型の値である true もしくは false に評価される式を書きます。
...
コメント部分の説明は、"int 関数に3を適用する"とも表せます。
...
パラメータ x と y の型を書いていない点に注目してください。
これは、レコードが型推論されることを示しています。

レコードの一部のフィールドの値はコピーし、一部の値は変更して新たな値を作りたい場合には、 @<kw>{with} キーワードを使います。
...
2-タプルの場合は、標準で提供されている fst 関数、 snd 関数でそれぞれ第一要素、第二要素が取り出せます。
...
3-タプルや 4-タプルなど、2-タプル以外のタプルでは fst 関数や snd 関数は使えないので注意してください。
