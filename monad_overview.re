...
型が存在するプログラミング言語では、何らかの状態を表したり、性質を持つことを型で表現します。
"何らかの状態を表す型"や"何らかの性質を持つ型"と呼ぶことを避けるため、以降ではこれらのことを@<kw>{文脈}と呼ぶことにします。
ここでは、ある文脈を @<code>{M<'T>} と表現することにしましょう。
@<code>{'T} は任意の型です。

'T は何かしらの手順 ── 手続きを経なければ、 @<code>{M<'T>} に変換できません。
'T を @<code>{M<'T>} に変換する手順を疑似コードで表すと、次のようになります。
...
次に、 'T に計算を適用しつつ 文脈 M に持ち上げる関数を考えてみましょう。

//emlist{
'T -> M<'U> // 'T に計算を適用した結果 'U を持ち上げる
//}

この関数に M<'T> を適用したい場合は、文脈から 'T を取り出しつつ、計算結果を再び文脈 M に持ち上げて返す手続きがあればよさそうです。
